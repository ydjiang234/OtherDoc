...
To avoid this error, you have to disguise your Linux distribution as one of the aforementioned release names. The Abaqus installation program calls a script named ``Linux.sh'' to detect your Linux release name. You have to change the line ``DSY\_OS\_Release=`lsb\_release --short --id |sed 's/ //g'`'' to ``DSY\_OS\_Release=``CentOS'''', as shown in Figure \ref{fig-2}. In the terminal, you can modify the files with software ``nano'' or ``vim''. By doing this, the installation program will detect your Linux release name as ``'Cent OS'.
...
It should be highlighted that you cannot start the installation program from the \ifp\ or its subdirectories. You can change your current directory with ``cd'' command.

\cmd{\ifp/1/StartGUI.sh}

When the installation program started, you can selected and install the SIMULIA products. To install Abaqus, the ``FLEXnet License Server'', ``Abaqus Simulation Services'', ``Abaqus Simulation Services CAA API'' and ``Abaqus/CAE'' need to be selected.
...
''. When ``FLEXnet License Server'' is installed, you need to start the FLEXnet Server before installing ``Abaqus/CAE''. To complete this, you need to open a new terminal (or new SSH connection) and start server with following command (if you install the ``FLEXnet License Server'' into the default directory).

\cmd{/usr/SIMULIA/License/2017/linux\_a64/code/bin/lmgrd}

During the installation of ``Abaqus/CAE'', the ``SIMULIA FLEXnet'' should be selected as the license server. The license server of NUI Galway is ``27004@cert1.srv.nuigalway.ie''. 

After the completion of installation, the ABAQUS licence type should be changed from academic to research. To do this, one of the Abaqus environment files, ``custom\_v6.env'', should be modified. If the Abaqus is installed into its default directory, the file can be edit with ``nano'' or ``vim''.

\cmd{nano /usr/DassaultSystemes/SimulationServices/V6R2017x/linux\_a64/SMA/site}
...
To avoid typing the full path to start Abaqus, the path ``/var/DassaultSystemes/SIMULIA\\/Commands'' needs to be added to the ``PATH'' variable of Linux. The ``PATH'' variable of the System is defined in the file ``/etc/environment''. Add the Abaqus command path into the file with ``nano'' or ``vim''.

\cmd{sudo nano /etc/environment}

After modifying the ``/etc/environment'' file, a reboot or re-login is required to make the modification into effects. Now the Abaqus can be started by simply calling:
